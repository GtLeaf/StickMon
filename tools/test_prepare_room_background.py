...
class RoomPackTests(unittest.TestCase):
    def test_room_pack_v2_header_size(self):
        self.assertEqual(room_assets.ROOM_PACK_VERSION, 2)
    ...
    def test_generated_standard_room_contains_doorway(self):
        pack = Path(__file__).resolve().parents[1] / "data/packs/dev/rooms/standard.smonroom"
        values = struct.unpack_from(room_assets.ROOM_PACK_HEADER_FORMAT, pack.read_bytes())
