...
constexpr uint16_t PHYSICAL_WIDTH = 466;
constexpr uint16_t PHYSICAL_HEIGHT = 466;
constexpr uint16_t CONTENT_PHYSICAL_WIDTH = LOGICAL_WIDTH * 2;
constexpr uint16_t CONTENT_PHYSICAL_HEIGHT = LOGICAL_HEIGHT * 2;
constexpr uint16_t PHYSICAL_OFFSET_X =
    (PHYSICAL_WIDTH - CONTENT_PHYSICAL_WIDTH) / 2;
constexpr uint16_t PHYSICAL_OFFSET_Y =
    (PHYSICAL_HEIGHT - CONTENT_PHYSICAL_HEIGHT) / 2;
...
void scaleStripe2x(const uint16_t* source, uint16_t sourceY,
    ...
    std::fill(destination,
              destination + static_cast<size_t>(PHYSICAL_WIDTH) *
                  (logicalRows * 2U), 0);
    for (uint16_t row = 0; row < logicalRows; ++row) {
        ...
        uint16_t* top = destination +
            static_cast<size_t>(row * 2U) * PHYSICAL_WIDTH + PHYSICAL_OFFSET_X;
...
esp_err_t submitFrame(esp_lcd_panel_handle_t panel,
    ...
    for (uint16_t sourceY = sourceBegin;
        ...
        result = esp_lcd_panel_draw_bitmap(
            // scaleStripe2x produces a full-width 466-pixel stripe and
            // places the 184x224 logical canvas in its centered region.
            panel, 0, physicalY + PHYSICAL_OFFSET_Y,
            PHYSICAL_WIDTH,
            physicalY + PHYSICAL_OFFSET_Y + physicalRows,
            transferBuffers[nextBuffer]);
        if (result == ESP_OK) {
            ++pendingTransfers;
...
esp_err_t submitFrameRegion(esp_lcd_panel_handle_t panel,
    ...
    uint16_t physicalXBegin = PHYSICAL_OFFSET_X + xBegin * 2U;
    uint16_t physicalXEnd = PHYSICAL_OFFSET_X + xEnd * 2U;
    esp_err_t result = ESP_OK;
    size_t pendingTransfers = 0;
    size_t nextBuffer = 0;
    for (uint16_t sourceY = sourceBegin;
        ...
        uint16_t physicalY = PHYSICAL_OFFSET_Y + sourceY * 2U;
        uint16_t physicalRows = logicalRows * 2U;
...
extern "C" void app_main(void) {
    ...
    Stickmon::ClawRuntime::instance().begin();
#endif
    app.render(canvas);
    app.markRendered();
    ...
    if (!touchReady) {
        ESP_LOGE(TAG, "CST9217 touch initialization failed");
    }
    ...
    ESP_LOGI(TAG, "Interactive home screen presented at 466x466 (184x224 centered)");
    while (true) {
