# 172 Pichu PMD processing spec

This spec is intentionally scoped to Pichu only.

## Source

- Input sheet: `origin_asset/source_sheets/low/1/皮丘.psd`
...
- Output root: `origin_asset/processed/172_pichu/`
...
- Note: Only the five left-side source directions are exported; right-side directions are mirrored at runtime.
- Note: This source sheet has no separate idle action; idle uses the first Idle/Moving frame (`walk0`).
- Note: Crop boxes were derived from PSD layer groups walk0..walk4 and layer sleeping.

## Contact sheet

- File: `pichu_idle_walking_contact.png`.
...
- Scope: only species `172` uses this Pichu state set.
