# 75 Graveler PMD processing spec

This spec is intentionally scoped to Graveler only.

## Source

- Input sheet: `origin_asset/source_sheets/low/075_graveler.png`
...
- Output root: `origin_asset/processed/075_graveler/`
...
- Note: The first 15 sprites on row 1 are walking frames: five directions, three frames each.
- Note: This sheet has no separate idle action; idle uses walking frame 0 for each direction.
- Note: Sleeping uses the seventh-last and sixth-last sprites on row 2, in left-to-right order.
- Note: Only five source directions are exported; right-side directions are mirrored at runtime.
- Note: A 64x68 canvas preserves fixed 2x scaling for the tallest walking poses.
- Note: Overlapping neighboring frame bounds are isolated by keeping the target connected component.

## Contact sheet

- File: `graveler_idle_walking_contact.png`.
...
- Scope: only species `75` uses this Graveler state set.
