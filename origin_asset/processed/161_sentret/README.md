# 161 Sentret PMD processing spec

This spec is intentionally scoped to Sentret only.
...
- Output root: `origin_asset/processed/161_sentret/`
...
- Note: A taller canvas preserves the fixed 2x scale of the upright tail without shrinking idle frames.

## Contact sheet

- File: `sentret_idle_walking_contact.png`.
- Top-left label: output frame size, `frame 64x76 px`.
...
- Scope: only species `161` uses this Sentret state set.
