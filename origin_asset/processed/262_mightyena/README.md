# 262 Mightyena PMD processing spec

This spec is intentionally scoped to Mightyena only.

## Source

- Input sheet: `origin_asset/source_sheets/medium/261_poochyena_262_mightyena.png`
...
- Output root: `origin_asset/processed/262_mightyena/`
...
- Note: Row 2 contains 15 moving frames: five directions, three frames each.
- Note: This sheet has no separate idle action; idle uses moving frame 0 for each direction.
- Note: Sleeping uses the third-last and second-last sprites on row 4, in left-to-right order.
- Note: Only five source directions are exported; right-side directions are mirrored at runtime.
- Note: A taller canvas preserves fixed 2x scaling for the 30-pixel-high moving poses.

## Contact sheet

- File: `mightyena_idle_walking_contact.png`.
...
- Scope: only species `262` uses this Mightyena state set.
