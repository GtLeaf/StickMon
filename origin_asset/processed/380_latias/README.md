# 380 Latias PMD processing spec

This spec is intentionally scoped to Latias only.

## Source

- Input sheet: `origin_asset/source_sheets/high/380_latias.png`
...
- Output root: `origin_asset/processed/380_latias/`
...
- Source row order: `left`, `down_left`, `front`, `up_left`, `back`.
- Exported direction order: `left`, `down_left`, `front`, `up_left`, `back`.
- Mirrored directions: right-side directions are drawn by runtime mirroring; only source direction PNGs are exported.

- Note: The sheet is horizontal; source direction order is left, down_left, front, up_left, back.
- Note: Only five source directions are exported; right-side directions are mirrored at runtime.

## Contact sheet

- File: `latias_idle_walking_contact.png`.
...
- Scope: only species `380` uses this Latias state set.
