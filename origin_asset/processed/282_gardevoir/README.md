# 282 Gardevoir PMD processing spec

This spec is intentionally scoped to Gardevoir only.

## Source

- Input sheet: `origin_asset/source_sheets/medium/282_gardevoir.png`
...
- Output root: `origin_asset/processed/282_gardevoir/`
...
- `idle`: 3 frames, indices `0..2`.
- `walking`: 3 frames, indices `0..2`.

- `sleeping`: 2 non-directional frames, indices `0..1`.

- `idle_alt`: 3 frames per direction; source directions are `front`, `down_left`, `left`, `up_left`, `back`. Extracted for future use and not wired into the current runtime state machine.
- `faint`: 1 frame per direction; source directions are `non_directional`. Extracted for future use and not wired into the current runtime state machine.
...
- Note: Row 1 contains walking frames: five directions, three frames each.
- Note: Walking is intended for a start-hold-end motion pattern like Haunter when wired into the runtime.
- Note: Row 2 is the primary three-frame idle action; row 3 is a second three-frame idle action reserved for selection.
- Note: The final three sprites on row 4 are sleeping frame 0, sleeping frame 1, and a non-directional fainted frame.
- Note: Only five source directions are exported; right-side directions are mirrored at runtime.

## Contact sheet

- File: `gardevoir_idle_walking_contact.png`.
- File: `gardevoir_idle_alt_contact.png`.
- File: `gardevoir_faint_contact.png`.
- Top-left label: output frame size, `frame 64x64 px`.
- Columns: exported directions in the order above.
- Rows: `idle_0`, `idle_1`, `idle_2`, `walking_0`, `walking_1`, `walking_2`, `sleeping_0`, `sleeping_1`.
...
- Scope: only species `282` uses this Gardevoir state set.
