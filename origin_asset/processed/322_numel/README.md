# 322 Numel PMD processing spec

This spec is intentionally scoped to Numel only.
...
- Output root: `origin_asset/processed/322_numel/`
...
- Note: Frames are selected from the sprite blocks immediately left of the Idle, Walking, and Sleeping labels.
- Note: Idle contains two frames per direction; walking contains three frames per direction.
- Note: Only five source directions are exported; right-side directions are mirrored at runtime.

## Contact sheet

- File: `numel_idle_walking_contact.png`.
...
- Scope: only species `322` uses this Numel state set.
