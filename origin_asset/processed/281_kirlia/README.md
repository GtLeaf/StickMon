# 281 Kirlia PMD processing spec

This spec is intentionally scoped to Kirlia only.

## Source

- Input sheet: `origin_asset/source_sheets/high/281_kirlia.png`
...
- Output root: `origin_asset/processed/281_kirlia/`
...
- `attacking`: 3 frames per direction; source directions are `front`, `down_right`, `right`, `up_right`, `back`. Extracted for future use and not wired into the current runtime state machine.
...
- Note: Walking contains two rows: row 1 is frame 0 and row 2 is frame 1.
- Note: The first five columns are front, down_left, left, up_left, back.
- Note: The sheet labels idle as the same spinning motion as walking, so idle reuses both walking frames.
- Note: Only five source directions are exported; right-side directions are mirrored at runtime.
- Note: Attacking contains three frames per direction and is reserved for future use.

## Contact sheet

- File: `kirlia_idle_walking_contact.png`.
- File: `kirlia_attacking_contact.png`.
- Top-left label: output frame size, `frame 64x64 px`.
- Columns: exported directions in the order above.
- Rows: `idle_0`, `idle_1`, `walking_0`, `walking_1`, `sleeping_0`, `sleeping_1`.
...
- Scope: only species `281` uses this Kirlia state set.
