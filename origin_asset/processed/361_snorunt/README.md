# 361 Snorunt PMD processing spec

This spec is intentionally scoped to Snorunt only.

## Source

- Input sheet: `origin_asset/source_sheets/high/361_snorunt.png`
...
- Output root: `origin_asset/processed/361_snorunt/`
...
- Note: The left four columns contain moving frames; each source row contains four frames.
- Note: This sheet has no separate idle action; idle uses moving frame 0 for each direction.
- Note: The sheet row order is front, down_right, right, up_right, back, up_left, left, down_left.
- Note: Right-side source rows are omitted; up_right, right, and down_right are mirrored from the corresponding left-side directions at runtime.
- Note: Sleeping uses the two sprites immediately above the Sleeping label.

## Contact sheet

- File: `snorunt_idle_walking_contact.png`.
...
- Scope: only species `361` uses this Snorunt state set.
