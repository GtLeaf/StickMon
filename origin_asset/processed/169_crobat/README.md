# 169 Crobat PMD processing spec

This spec is intentionally scoped to Crobat only.

## Source

- Input sheet: `origin_asset/source_sheets/medium/169_crobat.png`
...
- Output root: `origin_asset/processed/169_crobat/`
...
- Note: Row 1 contains 15 walking frames: five directions, three frames each.
- Note: This sheet has no separate idle action; idle uses walking frame 0 for each direction.
- Note: Sleeping uses the first two sprites on row 3.
- Note: Only five source directions are exported; right-side directions are mirrored at runtime.
- Note: A 68x68 canvas preserves fixed 2x scaling and the full wing tips.

## Contact sheet

- File: `crobat_idle_walking_contact.png`.
- Top-left label: output frame size, `frame 68x68 px`.
...
- Scope: only species `169` uses this Crobat state set.
