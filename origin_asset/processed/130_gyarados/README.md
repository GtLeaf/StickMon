# 130 Gyarados PMD processing spec

This spec is intentionally scoped to Gyarados only.

## Source

- Input sheet: `origin_asset/source_sheets/high/130_gyarados.png`
...
- Output root: `origin_asset/processed/130_gyarados/`
- Format: RGBA PNG with transparent background.
- Canvas: 88x82 px.
- Target scale: nearest-neighbor 1x, capped to fit the canvas without clipping.
...
- Note: The source sheet provides front, down_right, right, up_right, and back; left-side directions are generated by mirroring.
- Note: The source label says Moving; project assets store it as walking for state-machine compatibility.
- Note: Uses an 88x82 canvas so the large body poses are not shrunk into the default 64x64 frame.

## Contact sheet

- File: `gyarados_idle_walking_contact.png`.
...
- Scope: only species `130` uses this Gyarados state set.
