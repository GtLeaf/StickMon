# 280 Ralts PMD processing spec

This spec is intentionally scoped to Ralts only.

## Source

- Input sheet: `origin_asset/source_sheets/high/280_ralts.png`
...
- Output root: `origin_asset/processed/280_ralts/`
...
- Source row order: `front`, `down_right`, `right`, `up_right`, `back`.
- Exported direction order: `front`, `down_left`, `left`, `up_left`, `back`.
- Mirrored directions: `down_left` from `down_right`, `left` from `right`, `up_left` from `up_right`.

- Note: Idle contains two frames per direction and walking contains three frames per direction.
- Note: The first five source rows are front, down_right, right, up_right, back.
- Note: Left-side preview frames are mirrored from the corresponding right-side source rows.
- Note: The remaining three source rows are intentionally ignored.

## Contact sheet

- File: `ralts_idle_walking_contact.png`.
- Top-left label: output frame size, `frame 64x64 px`.
- Columns: exported directions in the order above.
- Rows: `idle_0`, `idle_1`, `walking_0`, `walking_1`, `walking_2`, `sleeping_0`, `sleeping_1`.
...
- Scope: only species `280` uses this Ralts state set.
