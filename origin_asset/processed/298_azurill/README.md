# 298 Azurill PMD processing spec

This spec is intentionally scoped to Azurill only.

## Source

- Input sheet: `origin_asset/source_sheets/medium/298_azurill.png`
...
- Output root: `origin_asset/processed/298_azurill/`
...
- `jump`: 3 frames per direction; source directions are `front`, `down_left`, `left`, `up_left`, `back`. Extracted for future use and not wired into the current runtime state machine.
...
- Note: This sheet has no separate idle action; idle uses moving frame 1 for each direction.
- Note: Row 2 contains 15 jumping frames: five directions, three frames each; these are reserved for future use.
...
- File: `azurill_idle_walking_contact.png`.
- File: `azurill_jump_contact.png`.
...
- Scope: only species `298` uses this Azurill state set.
