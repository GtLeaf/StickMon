...
- 资源预算报告：32 只 + 蛋图，原始 RGB565 1,196,032 B，RLE 后 403,980 B；当前固件 Flash 用量 1,398,249 B，RAM 47,984 B
...
当前实现基于 `doc/精灵图鉴检索.md`、Pokemon Essentials v21.1 的 `PBS/pokemon.txt` 与 `Graphics/Pokemon` 资源落地。首批名单必须同时存在于 PMD 图鉴检索表和 Essentials `Graphics/Pokemon`，并覆盖御三家、神兽、通讯进化。叶伊布、冰伊布虽有 Essentials 图片，但不在 PMD 图鉴检索表中，已从首批移除；当前用急冻鸟、闪电鸟、火焰鸟补齐神兽覆盖与 32 只规模。
...
- 精灵在房间内左右移动、停留、睡觉、靠近食物。
- v0.1 可互动项只做两类：摸一摸、加食物。
