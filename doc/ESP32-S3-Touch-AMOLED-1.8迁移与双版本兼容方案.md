...
| 资源包、解压、字体 | 直接共享 | 全屏背景按布局 profile 分包 |
...
- AMOLED 资源分区包含 StickS3 共用的 `game/ui.smonfx`，背包和商店通过
  ...
  提前恢复路线。
- 当前背包已支持回复药、状态治疗药和复活药。糖果、进化石、除虫喷雾、蜂蜜和
  心之鳞片依赖尚未迁入的成长/进化/探索效果/招式回忆流程，因此可见但不会扣除
  库存，避免出现半迁移状态的数据损失。
...
- 主界面锁屏和菜单按钮已从右上角迁到底部 HUD 左侧，顶部只保留品牌、游戏
  ...
  反馈仍只刷新 `172..224` 行。
- 已接通主菜单“房间”及其“食物、洗澡、返回”子页，其中食物选择页暂保留
  占位，洗澡已可完整操作。浴室背景、三种肥皂、刷子、花洒、泡沫和精灵正面图
  直接复用 `ui.smonfx` 与 `.smonsp` 资源，不维护第二套素材。
- 洗澡库存和数值规则已提取到共享 `game/BathService`：StickS3 与 AMOLED 共用
  三种肥皂消耗、每日照护经验上限、起泡/刷洗/冲洗三阶段经验和心情奖励。
  StickS3 原有升级、进化和招式学习仍由 `GameEngine` 结算；AMOLED 当前同步更新
  经验、等级和 HP，并记录待显示的升级状态，竖屏成长演出后续迁移。
...
- M5StickS3 PlatformIO release 构建和 AMOLED V1 ESP-IDF 构建均已通过。
...
- 继续迁移队伍的详细状态/招式管理页、房间食物子菜单、电脑和设置的业务视图。
- 迁移 AMOLED 升级、进化和招式学习演出，补齐睡眠姿态及更完整的移动/进食动画。
- 在现有路线行走上继续迁入拾取物、随机遭遇、路线阻挡、战斗、奖励和结算。
  当前路线只在本地演示行走，不修改探索进度，也不发放物品或经验。
- 接入 AXP2101、QMI8658、PCF85063A、ES8311 和真正的低功耗休眠。
- 实现 ESP-IDF 版 ESP-NOW 传输适配器并完成跨设备联调。
- 新建 V2（CO5300 + CST820）板级工程；在取得 V2 实机前只声明编译结果，不声明硬件通过。
