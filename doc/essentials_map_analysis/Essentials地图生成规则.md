...
霜晶窟的首层入口与末层出口使用 `Caves.png` 的亮光模块 `1299/1300/1301`，按四个边界预生成旋转后的资源，顶部入口额外使用 180° 版本；洞穴内部停用 `1308`，层间只使用贴合岩壁的 `1332＋1340` 上行梯子与 `1333` 下行梯子，暂不生成 `1321 -> 1322` 的坠落路线。同一入口方向有四种路线变体，再按种子铺设雪地装饰。冰面 `1328` 使路线连续滑行。预览多层效果：`python tools/generate_frost_crystal_cave_preview.py --levels`。
...
- 运行时算法 v9 将区域编号 `5` 映射为“古木瀑谷”。根据本层入口的上、右、下、左方向选择对应的已验证布局，均保留一个真实分叉点、两个不同边缘出口、两格宽路线、瀑布缺口和 Cave 岩石台阶；随机拓扑仍由 map seed 决定。
- 古木瀑谷合成顺序固定为“Cave 地面 `4758` -> Sea 上下游 -> 瀑布顶/身/底 -> Cave 悬崖 `4759/4760/4761` 与岩石台阶 `4705` -> 水中石块 -> Cave 洞口 `4700～4704`”。路线保留在拓扑/通行语义中，但不额外绘制 Outside 绿色道路；路线不得覆盖水体或 Cave 悬崖，楼梯格例外。该场景不放 Outside 顶边森林、普通草地、花簇或灌木。
